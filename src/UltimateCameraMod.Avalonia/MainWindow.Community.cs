...
public partial class MainWindow : Window
{
    private void OnBrowsePresetCatalog(object sender, RoutedEventArgs e)
    {
        ...
        else
        {
            ...
            {
                Dispatcher.UIThread.Invoke(() => RefreshPresetManagerLists(preserveSelection: true));
            },
        ...
        try
        {
            ...
            Dispatcher.UIThread.Invoke(() =>
            {
                if (isOutdated)
                {
                    ...
                    VersionUpdateBtn.IsVisible = true;
                }
                else
                {
    ...
    private void RefreshGameUpdateNotice()
    {
        try
        {
            ...
            {
                if (IsLoaded) GameUpdateStrip.IsVisible = false;
                return;
            }
            ...
            if (!ev.ShowWarning)
            {
                ...
                if (!_gameUpdateNoticeSessionDismissed)
                    GameUpdateStrip.IsVisible = false;
                return;
            }
