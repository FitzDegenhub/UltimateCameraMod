...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        {
            IntPtr shellHwnd = new (this).Handle;
            ...
            Dispatcher.UIThread.InvokeAsync(ApplyNativeWindowIcons, DispatcherPriority.Loaded);
            Dispatcher.UIThread.InvokeAsync(ApplyNativeWindowIcons, DispatcherPriority.Render);
            Dispatcher.UIThread.InvokeAsync(ApplyNativeWindowIcons, DispatcherPriority.ApplicationIdle);
        };
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                var (detectedPath, platform) = GameDetectorFactory.Create().FindGameDir();
    ...
    private void ShowWelcomeVerifyScreen()
    {
        ...
        yesBtn.Click += (_, _) =>
        {
            ...
            TutorialCanvas.IsVisible = false;
    ...
    private void OnGameDirResolved()
    {
        ...
        if (_detectedPlatform == "Xbox/GamePass" && !GameDetectorFactory.Create().CheckWritePermission(_gameDir))
        {
    ...
    private void ApplyDarkTitlebar()
    {
        try
        {
            var hwnd = new (this).EnsureHandle();
            int value = 1;
    ...
    private void SetGlobalBusy(bool visible, string? message = null)
    {
        void Apply()
        {
            ...
            if (!visible)
            {
                GlobalBusyOverlay.IsVisible = false;
                return;
            }

            if (!string.IsNullOrEmpty(message))
                GlobalBusyText.Text = message;
            GlobalBusyOverlay.IsVisible = true;
        }
