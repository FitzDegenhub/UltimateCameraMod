...
public partial class CommunityBrowserDialog : UserControl
{
        ...
        bool needsSessionXmlBake = false)
    {
        ...
        AttachedToVisualTree += async (_, _) => await FetchCatalogAsync();
    }

    // Avalonia requires a parameterless constructor for the XAML loader
    public CommunityBrowserDialog() : this("", () => { }) { }
    ...
    private Border BuildPresetCard(CatalogEntry entry, bool isDownloaded)
    {
        var nameBlock = new TextBlock
        {
            ...
            Foreground = (IBrush?)this.FindResource("TextPrimaryBrush") ?? Brushes.White
        };
        ...
        var authorBlock = new TextBlock
        {
            ...
            FontSize = 10,
            Foreground = (IBrush?)this.FindResource("TextDimBrush") ?? Brushes.Gray,
        ...
        var descBlock = new TextBlock
        {
            ...
            Foreground = (IBrush?)this.FindResource("TextSecondaryBrush") ?? Brushes.LightGray,
        ...
        foreach (string tag in entry.Tags)
        {
            ...
            {
                Background = (IBrush?)this.FindResource("BgInputBrush") ?? Brushes.DarkGray,
        ...
        if (isDownloaded)
        {
            ...
            actionBtn.Classes.Add("subtle");
            actionBtn.IsEnabled = false;
        }
        else
        {
            actionBtn.Content = "Download";
            actionBtn.Classes.Add("accent");
        ...
        if (!string.IsNullOrWhiteSpace(entry.Url))
        {
            var linkBtn = new Button
            {
                ...
                MinWidth = 110,
                HorizontalAlignment = HorizontalAlignment.Right,
                Foreground = (IBrush?)this.FindResource("AccentBrush") ?? Brushes.Gold,
                BorderBrush = (IBrush?)this.FindResource("AccentBrush") ?? Brushes.Gold,
                Margin = new Thickness(0, 6, 0, 0),
                Tag = entry.Url
            };
            linkBtn.Classes.Add("subtle");
            linkBtn.Click += (_, _) =>
            {
        ...
        return new Border
        {
            Background = (IBrush?)this.FindResource("BgPanelBrush") ?? Brushes.DarkGray,
            BorderBrush = (IBrush?)this.FindResource("BorderBrush") ?? Brushes.Gray,
    ...
    private void ShowLoading(string text)
    {
        ...
        PresetListScroller.IsVisible = false;
    }
    ...
    private void ShowContent()
    {
        LoadingPanel.IsVisible = false;
        ErrorPanel.IsVisible = false;
        PresetListScroller.IsVisible = true;
    }
